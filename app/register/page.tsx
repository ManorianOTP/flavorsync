import { Container, Paper, TextInput, PasswordInput, Button, Title, Text, Anchor, Stack, Divider } from '@mantine/core';
import Link from 'next/link';
import classes from './register.module.css';

export default function RegisterPage() {
  return (
    <Container size="xs" className={classes.container}>
      <Paper radius="md" p="xl" withBorder className={classes.form}>
        <Title order={2} className={classes.title} ta="center" mt="md" mb={50}>
          Create your FlavorSync account
        </Title>

        <Stack>
          <Button variant="default" radius="xl" className={classes.providerButton}>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ marginRight: 10 }}>
              <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/>
              <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
              <path d="M5.84 14.1c-.22-.66-.35-1.36-.35-2.1s.13-1.44.35-2.1V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22.81-.62z" fill="#FBBC05"/>
              <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
            </svg>
            Continue with Google
          </Button>
        </Stack>

        <Divider label="Or continue with email" labelPosition="center" my="lg" />

        <form>
          <Stack>
            <TextInput
              label="Name"
              placeholder="Your name"
              radius="md"
              required
            />
            
            <TextInput
              label="Email"
              placeholder="hello@flavorsync.com"
              radius="md"
              required
            />

            <PasswordInput
              label="Password"
              placeholder="Your password"
              radius="md"
              required
            />

            <PasswordInput
              label="Confirm Password"
              placeholder="Confirm your password"
              radius="md"
              required
            />

            <Button type="submit" radius="xl" color="icon">
              Create account
            </Button>
          </Stack>
        </form>

        <Text ta="center" mt="md">
          Already have an account?{' '}
          <Anchor component={Link} href="/login" fw={700} c="icon">
            Sign in
          </Anchor>
        </Text>
      </Paper>
    </Container>
  );
} 